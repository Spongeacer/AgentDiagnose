...
class VerbNounEmbeddingStore:
    ...
    def _ingest_trajectory(self, trajectory, trajectory_id: str) -> Dict[str, int]:
        """Add all verb/noun/pair entries from a Trajectory into the store."""
            ...
            if action.output_verb_noun_pairs:
                for pair in action.output_verb_noun_pairs:
                    if isinstance(pair, (list, tuple)) and len(pair) == 2:
                        verb, noun = pair
                        if verb and noun and str(verb).strip() and str(noun).strip():
                            pair_text = f"{str(verb).strip()} {str(noun).strip()}"
    ...
    def process_trajectory_file(self, file_path: str) -> Dict[str, int]:
        ...
        counts = self._ingest_trajectory(trajectory, trajectory_id)
        return counts

    def process_trajectory_jsonl(self, jsonl_path: str, print_stats: bool = False) -> Dict[str, int]:
        """Process a Ducc/Claude-Code JSONL file using tool-call-based verb-noun extraction.

        Trajectories are extracted on-the-fly without writing intermediate JSON files,
        making this more efficient than the two-step generate_verb_nouns -> generate_embeddings
        pipeline for large JSONL files.
        """
        ...
        for raw in tqdm(lines, desc="Processing ducc trajectories"):
            raw = raw.strip()
            if not raw:
                continue
            try:
            ...
            try:
                ...
                if print_stats:
                    print(f"  [skip] {data_id}: {e}")
                n_skipped += 1
                continue

            counts = self._ingest_trajectory(trajectory, data_id)
            for key in total_counts:
                total_counts[key] += counts[key]

        if n_skipped:
            print(f"  Skipped {n_skipped} malformed records")
        if print_stats:
...
def create_embedding_store_from_jsonl(
    jsonl_path: str,
    output_dir: str,
...
) -> VerbNounEmbeddingStore:
    """Build embedding store directly from a Ducc/Claude-Code JSONL file."""
    store = VerbNounEmbeddingStore(
        model_name=model_name,
        faiss_index_type=faiss_index_type,
        model_kwargs=model_kwargs,
    ...
    counts = store.process_trajectory_jsonl(jsonl_path, print_stats=print_stats)

    if counts["verbs"] == 0 and counts["nouns"] == 0 and counts["pairs"] == 0:
        print("Warning: No verb-noun data found!")
        return store
...
def main():
    ...
    parser.add_argument("--input-jsonl",
                        help="Ducc/Claude-Code JSONL file (alternative to --input-dir)",
                        default=None)
    ...
    if args.input_jsonl:
        print(f"Loading from Ducc JSONL: {args.input_jsonl}")
        store = create_embedding_store_from_jsonl(
