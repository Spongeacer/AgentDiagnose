...
from evaluator.scorers.navigation_path_scorer_code_type import NavigationPathScorer_CodeType
...
def load_ducc_jsonl(file_path: str, limit: Optional[int] = None) -> Dict[str, Trajectory]:
    """Load trajectories from a ducc JSONL file."""
    trajectories = {}
    with open(file_path, 'r', encoding='utf-8') as f:
        for i, line in enumerate(tqdm(f, desc="Loading ducc trajectories")):
            ...
            try:
                ...
                traj = Trajectory.from_ducc_record(record)
                if traj.objective and traj.actions:
                    traj_id = traj.id or f"traj_{i:04d}"
...
def evaluate_trajectories(trajectories: Dict[str, Trajectory],
    ...
    models = {
        ...
        "navigation_path_code_type": "no_model_needed",
    }
...
def parse_args():
    ...
    parser.add_argument("--input", "-i", required=True, help="Directory containing trajectory JSON files, or a JSONL file")
    parser.add_argument("--input-format", choices=["json", "ducc_jsonl"], default="json",
                        help="Input format: 'json' (default, directory of JSON files) or 'ducc_jsonl' (single JSONL file)")
...
def main():
    ...
    if args.input_format == "ducc_jsonl":
        trajectories = load_ducc_jsonl(args.input, limit=args.limit)
    else:
